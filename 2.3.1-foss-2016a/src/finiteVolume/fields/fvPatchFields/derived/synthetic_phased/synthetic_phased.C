...
synthetic_phased::synthetic_phased
(
    const fvPatch& p,
	...
	z_(dict.lookup("z"))
{
    ...
    {
        FatalErrorIn("synthetic_phased(dict)")
	...
	Info<< "Version synthetic_phased." << endl;
...
void synthetic_phased::updateCoeffs()
{
    if (updated())
    {
        return;
...
*/
	...
	Info<< "omega * t : " << omega * t << endl;
	Info<< "omega * t + phi0_ : " << omega * t + phi0_ << endl;
	Info<< "sin(omega * t + phi0_) : " << sin(omega * t + phi0_) << endl;
	Info<< "1 + a * sin(omega * t + phi0) : " << 1 + a * sin(omega * t + phi0) << endl;
	Info<< "uTau = " << uTau << endl;
...
void synthetic_phased::write(Ostream& os) const
{
...
makePatchTypeField(fvPatchVectorField, synthetic_phased); // this is important too. maybe creating the real class member
