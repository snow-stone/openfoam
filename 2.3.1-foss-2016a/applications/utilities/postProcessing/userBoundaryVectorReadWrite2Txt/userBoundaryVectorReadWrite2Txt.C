	...
	forAll(timeDirs, timeI)
	{
	    ...
	    if (header.headerOk())
	    {
			...
			if (patchLabel != -1)
			{
				...
				vectorField& s_FieldOnPatch = s.boundaryField()[patchLabel];
				Info<< "On patch " << patchName << endl;

				Info<< "field " << fieldName << " component X "<< endl;
				scalarField_simpleStatistics(s_FieldOnPatch.component(vector::X));
			...
			if (!noWriting)
			{
				...
				if (patchLabel != -1)
				{
        			...
        			{
        			    txtOutput
        				    //<< s.boundaryField()[patchLabel][i] // not implemented here. output a vector
        				    << s.boundaryField()[patchLabel][i].component(vector::X) // a scalar it is self-evident how to output to a file
        				    << std::endl;
        			}
				}
				else
