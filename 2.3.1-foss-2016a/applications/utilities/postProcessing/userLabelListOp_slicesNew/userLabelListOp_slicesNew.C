...
int main(int argc, char *argv[])
{
	...
	{ 
	    ...
	    (
		    fileName(string("userDefinedLog")/string(scalarFieldName+"_"+sliceName+"_mean_rmsNew")).c_str(),
		    ios_base::app
	    );
		...
		forAll(slice, i)
		{
			TMeanSpatial += slice_T[i] * magSf.boundaryField()[patchLabel][i];
		}
		TMeanSpatial /= S;
		...
		forAll(slice, i)
		{
			TVARSpatial += sqr(slice_T[i] - TMeanSpatial) * magSf.boundaryField()[patchLabel][i] ;
		}
		TVARSpatial /= S;
